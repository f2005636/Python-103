...
data rg.c3 (drop = mh10 mh32);
set rg.c3 (rename = (
Medical_History_10 = mh10
Medical_History_32 = mh32
));
format Medical_History_10 best12.;
format Medical_History_32 best12.;
Medical_History_10 = input(mh10,8.);
Medical_History_32 = input(mh32,8.);
run;
...
data rg.df_test(drop = flag row_num y); set rg.c3; 
where flag = "test";
run; 

%macro rg_export (p);
proc export data=rg.&p. outfile="/folders/myfolders/&p..csv" 
dbms=csv replace;
run;
%mend;
%rg_export(df_train1);
